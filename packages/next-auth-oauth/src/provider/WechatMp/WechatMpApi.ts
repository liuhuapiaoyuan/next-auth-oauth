...
export class WechatMpApi {
  ...
  encryptMessage(xml: string) {
     ...
     // 拼接 FullStr
     const FullStr = Buffer.concat([random, msg_len, msg, Buffer.from(this.appId, 'utf-8')]);
     // 3. PKCS7 填充
     const blockSize = 32; // AES 密钥长度
     const paddedFullStr = padPKCS7(FullStr, blockSize);
     // 4. 使用 AESKey 加密
     const cipher = crypto.createCipheriv('aes-256-cbc', AESKey, AESKey.subarray(0, 16)); // IV 为 AESKey 的前 16 字节
